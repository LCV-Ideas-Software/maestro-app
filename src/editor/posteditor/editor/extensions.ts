      ...
      increaseIndent:
        ...
        ({ tr, state, dispatch }: any) => {
          ...
          state.doc.nodesBetween(from, to, (node: any, pos: number) => {
            ...
            const next =
              idx < INDENT_LEVELS.length - 1
                ? INDENT_LEVELS[idx + 1]
                : INDENT_LEVELS[INDENT_LEVELS.length - 1];
            if (next !== current && dispatch)
              tr.setNodeMarkup(pos, undefined, { ...node.attrs, textIndent: next });
          });
          return true;
        },
      decreaseIndent:
    ...
    return {
      ...
      width: {
        ...
        renderHTML: (attributes) => {
          ...
          const normalized = String(attributes.width).endsWith("%")
            ? attributes.width
            : `${attributes.width}`;
...
export const FigureImageNode = TiptapNode.create({
  ...
  renderHTML({ HTMLAttributes }) {
    ...
    const figureStyle =
      width && width !== "100%" ? `width: ${width}; max-width: 100%` : "max-width: 100%";
    return [
      "figure",
...
export const buildTiptapExtensions = (mentionItems: string[]) => [
  ...
  CustomResizableYoutube.configure({
    inline: false,
    width: 840,
    height: 472,
