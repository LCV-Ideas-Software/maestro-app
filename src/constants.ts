...
export const initialAgents: AgentCard[] = [
  ...
  { name: "Perplexity", cli: "perplexity-api", state: "blocked", note: "aguardando chave de API" },
  {
    ...
    cli: "motor local",
    state: "blocked",
    note: "aguardando verificacoes iniciais",
  },
];
...
export const initialDiscussionRounds: DiscussionRound[] = [
  {
    round: "--",
    status: "Sem rodada",
    note: "Submeta um prompt para criar a primeira ata operacional.",
  },
];
...
export const credentialStorageModes = [
  ...
  { mode: "windows_env", label: "Env var Windows", detail: "segredos em env var; configs em JSON" },
  {
    mode: "cloudflare",
    label: "Cloudflare",
    detail: "maestro_db + Secrets Store remoto (execucao local exige MAESTRO_*_API_KEY em env)",
  },
] satisfies Array<{ mode: CredentialStorageMode; label: string; detail: string }>;

export const storageModeSummaries: Record<
  CredentialStorageMode,
...
> = {
  ...
  windows_env: {
    ...
    detail:
      "Tokens e API keys ficam em env vars do Windows; demais configuracoes ficam em JSON local.",
  },
  cloudflare: {
...
export const idleActivityFeed: ActivityItem[] = [
  {
    ...
    detail:
      "Logs estruturados ativos. Ao submeter um prompt, o monitor deve registrar cada etapa visivel.",
  },
  {
    level: "diagnostic",
    time: "--:--:--",
