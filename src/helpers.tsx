...
export function isTextLikeAttachment(attachment: PromptAttachmentPayload) {
  ...
  return [
    ...
    "yml",
    "log",
  ].includes(attachmentExtension(attachment.name));
}
...
export function isImageAttachment(attachment: PromptAttachmentPayload) {
  return ["image/png", "image/jpeg", "image/webp", "image/gif"].includes(
    normalizedAttachmentMediaType(attachment),
  );
}

export function isPdfAttachment(attachment: PromptAttachmentPayload) {
  return (
    normalizedAttachmentMediaType(attachment) === "application/pdf" ||
    attachmentExtension(attachment.name) === "pdf"
  );
}
...
export function attachmentDeliveryHint(plan: AttachmentDeliveryPlan) {
  ...
  if (plan.manifestProviders.length > 0) {
    ...
    parts.push(
      `Manifesto/previews: ${plan.manifestProviders.map(providerShortLabel).join(", ")}${reason}`,
    );
  }
...
export function humanizeAgentStatus(status: string) {
  ...
  if (normalized.startsWith("PROVIDER_INCOMPLETE_RESPONSE"))
    return "Resposta incompleta do provedor";
  ...
  if (normalized === "PAUSED_LEGACY_RETRY_ACCOUNTING_UNKNOWN")
    return "Historico de custo da sessao incompleto";
...
export function summarizeAgentResults(agents: EditorialAgentResult[]) {
  ...
  const latestText = latest
    .map((agent) => `${agent.name}: ${humanizeAgentStatus(agent.status)}`)
    .join("; ");
  const failures = agents.filter(
    (agent) => agent.tone === "error" || agent.tone === "blocked",
  ).length;
