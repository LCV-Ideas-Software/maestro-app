...
Frontend command/panel coverage runs locally. Rust unit, Clippy, and build validation run in GitHub Actions; the operator's 25/09/2026 decision also permits local Rust validation with an allowed toolchain. The final audit compares non-Latin author keys without discarding their letters, detects narrative citations in other scripts, decomposes grouped author-date citations, and requires one body occurrence per manifest entry. It checks the declared citation type, an explicit final reference section and unique reference lines. The maintained `pulldown-cmark` CommonMark parser identifies code and raw HTML boundaries so code examples do not trigger citation signals.
