...
Frontend command/panel coverage runs locally. Rust unit, Clippy, and build validation run in GitHub Actions; the operator's 25/09/2026 decision also permits local Rust validation with an allowed toolchain. The final audit compares non-Latin author keys without discarding their letters, detects narrative citations in other scripts, decomposes grouped author-date citations, and requires one body occurrence per manifest entry. Citation type expresses the claim's meaning: an indirect quotation or paraphrase may use narrative or parenthetical author-date form. A generic mention must use narrative form, and each `apud` occurrence must name the verified consulted source in the text. A direct quotation must have its own adjacent citation, separated only by punctuation or whitespace; it requires a locator even when using `apud`, plus actual access to the consulted document or excerpt. The audit requires an explicit final reference section and unique reference lines. The maintained `pulldown-cmark` CommonMark parser identifies code and raw HTML boundaries so code examples do not trigger citation signals.

The final reader audits rendered body text across the whole document, excluding only the actual final-reference lines. It locates top-level reference headings with the CommonMark parser, so visible text such as `#Referencias` or a nested heading in a quote or list cannot hide later citations. It builds the visible projection from CommonMark text events, leaving all markup delimiters blank at their source offsets. It decodes CommonMark escapes and entities once, then checks citations, quotations, and unstructured signals on that same projection; it ignores link destinations, link titles, image alt text, raw HTML, and code, and still inspects later sections after References. A reference heading without a manifest also blocks release. Citation occurrences are matched one-to-one against compatible manifesto entries before claims are consumed; each source must match a distinct normalized line inside References. The reader fails closed on capacity overflow. Quotations of two or more words need an adjacent structured citation; an overlong quotation is blocked explicitly instead of silently skipped. The University of Sao Paulo's [ABNT NBR 10520:2023 citation guide](https://sddarquivos.webhostusp.sti.usp.br/arquivos/Guia_Citacoes_ABNT10520_FOB-USP.html) supplies the checked three-author narrative and optional four-or-more-author `et al.` forms.
