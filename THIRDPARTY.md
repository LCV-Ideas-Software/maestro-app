...
The npm half of that bundle is no longer a snapshot. Vite's official
`build.license` capability now emits `legal/BUNDLED-LICENSES.md` on every build,
from the dependency graph the bundler actually packages, and the frontend bundle
carries a banner pointing at it. Because Tauri embeds the built frontend in the
distributed binary, that report ships with the artifact and is regenerated by the
build rather than maintained by hand (LCV-207).

**The Rust half is not covered by it.** `build.license` sees only what the
JavaScript bundler packages; the crates under `src-tauri/` remain the preserved
snapshot in `THIRD-PARTY-NOTICES.txt` and still require manual review before a new
