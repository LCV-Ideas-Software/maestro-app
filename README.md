...
When a reviewer revises approved text, the content lock reads the isolated `<maestro_revision_report>` body as one strict JSON object. Each `changed_blocks` entry must name an existing received block and provide its own protocol basis for edits. Exact `change_type` values authorize a reorder or a bounded number of added blocks; free-text `reason` does not grant permission. Malformed reports and ambiguous edits to identical received blocks fail closed. See [editorial session workflow](./docs/editorial-session-workflow.md#approved-content-lock).
