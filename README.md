...
When a reviewer revises approved text, the content lock reads the isolated `<maestro_revision_report>` body as one strict JSON object. Each `changed_blocks` entry must name an existing received block and provide a substantive protocol basis for edits. Exact `change_type` values authorize a reorder or a bounded number of added blocks from one local source; either immediately adjacent received block can anchor a pure insertion. Growth across multiple edited received blocks in one region requires separate serial turns because the revised blocks have no provenance IDs. Free-text `reason` does not grant permission. Malformed reports and edits to repeated text without identifiable origin fail closed. See [editorial session workflow](./docs/editorial-session-workflow.md#approved-content-lock).
...
- **Automation authority**: same-repository Dependabot PRs enable native GitHub auto-merge with squash against their exact head SHA, including standalone major updates. Required checks and inherited Enterprise protections remain authoritative; no merge queue or central controller is used. Version updates run every day, including weekends, at 05:00 (UTC-03:00), with the native `cron` schedule in `Etc/GMT+3` and the existing version groups. Cargo version updates cover direct and indirect dependencies. Security fixes have separate groups for each configured ecosystem.
- **Validation**: CI retains frontend tests, Biome, build, public HTML formatting and the local PostEditor compatibility snapshot. Rust checks, tests and Clippy run on GitHub-hosted Windows runners and may also run locally with an allowed toolchain under the operator's 25/09/2026 decision. CodeQL uses Default Setup for Actions, JavaScript/TypeScript and Rust; native Code Quality, Dependency Review, Zizmor and Scorecard remain separate controls.
