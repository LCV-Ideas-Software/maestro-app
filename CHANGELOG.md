...
- **MAESTRO-34 final engineering review:** Bind references to original CommonMark heading offsets before masking hidden text, exclude hidden HTML from citation occurrence matching, preserve balanced Markdown link destinations and exact redirect identity, ignore code/comment URL examples without hiding adjacent live anchors, trim long punctuation tails in linear time, reject encoded and composite credential-like URL keys and paths, fail closed when HTTP 304 cannot reuse or refresh its cache, verify persisted evidence artifacts, and suspend automated collection when `robots.txt` returns HTTP 429.
