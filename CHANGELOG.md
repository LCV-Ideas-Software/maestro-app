...
- **MAESTRO-34 final engineering review:** Bind references to top-level CommonMark heading offsets, check citation signals on the same rendered text without reparsing decoded entities, and block reference apparatus without a manifest. Exclude hidden HTML from citation occurrence matching, preserve balanced Markdown link destinations and exact redirect identity, ignore code/comment URL examples without hiding adjacent live anchors, trim long punctuation tails in linear time, prevent `mailto:` addresses without content hashes from being accepted as claim evidence, reject encoded and credential-like `*key`/`*sig` URL names while retaining ordinary anchors, fail closed when HTTP 304 cannot reuse or refresh its cache, verify persisted evidence artifacts, and suspend automated collection when `robots.txt` returns HTTP 429.
...
- **MAESTRO-30 — approved-content lock.** Parse the isolated revision report as
  ...
  when it exists in the received manifest. Require one local growth source on
  either adjacent received block or the changed block in that region, with its
  own growth limit; accept repeated text edits only when stable surrounding
  blocks identify the occurrence. Require substantive protocol basis even in
  structured values. Allow compound operation tokens, handle a unique edited
  source between reordered received neighbors, and require duplicate-free JSON
  even for unchanged serial turns. Keep the 19 original Rust test functions and add regressions
...
- **MAESTRO-17 — official Linear Release writer.** The continuous release
  ...
  v0.17.1 commit, and explicitly selects CLI v0.17.1 while preserving the
  existing environment, least-privilege token, complete Git history and
  serialized `main` synchronization. The official installer verifies the
  downloaded CLI against its published SHA-256, closing the artifact-integrity
  gap tracked in `linear/linear-release-action#59`; writer failures remain
  visible instead of being converted into a successful run.
...
- **MAESTRO-22 — release authorization no longer expires before normal Rust
  ...
  five minutes while valid CI and CodeQL Rust jobs are still running. Each
...
- Gates recorded after run: `cargo check --locked --all-targets`, `cargo test --locked`, `cargo clippy --locked --no-deps --all-targets`, `git diff --check`, `npm run biome`, `npm run typecheck`, `npm test`, `npm run build`. A pre-commit cross-review session was opened for this change; its identifier and progress are tracked in the local app memory file.
