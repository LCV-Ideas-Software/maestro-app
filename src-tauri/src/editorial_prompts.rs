...
2. `<maestro_revision_report>` containing exactly one valid en_US JSON object (no prose, Markdown fence, YAML, duplicate fields, or trailing text):
   ...
   - `current_author`
   - `status`
   - `changed_blocks`: list every changed received block using `block_id`, `change_type`, `reason`, `protocol_basis`, and `required: true|false`. Each entry names one received block exactly once. For a pure addition in a region with no edit, name either immediately adjacent existing received block; never invent a new `block_id`. For growth in a region with one edited received block, declare `"split"` or `"addition"` on that block; an unchanged neighbor cannot establish which revised text came from the edit. Exactly one local source must authorize each insertion. If two or more received blocks change in the same region, revise them first and add new blocks in a separate serial turn; their revised text has no provenance IDs to assign the growth safely. Each entry permits one extra block unless `new_block_count` explicitly gives a larger positive integer of EXTRA blocks. Use `"reorder"` whenever approved blocks move. When one block requires multiple operations, use a JSON array of distinct exact tokens, for example `"change_type": ["addition", "reorder"]`.
